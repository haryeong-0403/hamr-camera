...
class Camera0Publisher(Node):
    def __init__(self):
        super().__init__('camera3_publisher')

        # /dev/video1 오픈
        self.cap = cv2.VideoCapture(1)
        if not self.cap.isOpened():
            self.get_logger().error("Cannot open /dev/video1")
            exit(1)

        # 퍼블리셔 생성
        self.pub_raw = self.create_publisher(Image, '/camera3/image_raw', 10)
        self.pub_compressed = self.create_publisher(CompressedImage, '/camera3/image_raw/compressed', 10)
    ...
    def loop(self):
        while rclpy.ok():
            ...
            if not ret:
                self.get_logger().warn("Failed to read frame from /dev/video1")
                continue
...
# ----------------------------------------------------------------------------------------------- #
    ...
    def calc_fps(self):
        ...
        if self.frame_count >= 30:
            ...
            self.get_logger().info(f"[camera3] Average FPS: {fps:.2f}")
            self.last_time = now
            self.frame_count = 0
# ----------------------------------------------------------------------------------------------- #
