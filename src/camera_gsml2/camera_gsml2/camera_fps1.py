...
class Camera0Publisher(Node):
    def __init__(self):
        ...
        if not self.cap.isOpened():
            self.get_logger().error("Cannot open /dev/video3")
            exit(1)

        # 퍼블리셔 생성
        self.pub_raw = self.create_publisher(Image, '/camera1/image_raw', 10)
        self.pub_compressed = self.create_publisher(CompressedImage, '/camera1/image_raw/compressed', 10)
    ...
    def loop(self):
        while rclpy.ok():
            ...
            if not ret:
                self.get_logger().warn("Failed to read frame from /dev/video3")
                continue
